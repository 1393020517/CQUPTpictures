...
//        if (!is_dir($insert_path)) { //判断目录是否存在 不存在就创建,考虑如何在一段时间里面去建一个文件夹
//            mkdir($insert_path, 0777, true);
//        }
...
//            $extract2 = mysqli_query($a,"SELECT saved FROM USERINFO WHERE ID='$id'");
            ...
            if(!($aff > 0)){
                ...
                mysqli_query($a,"UPDATE tempphotos SET label='".$label."' WHERE paths='".$imageSrc."'");
//            if($extract2){      //原来有路径上传
